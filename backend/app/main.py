...
async def upload_pdf_notice(file: UploadFile = File(...)):
    ...
    if not file.filename.endswith(".pdf"):
        raise HTTPException(
            status_code=400, detail="Only PDF file extensions are supported."
        )
    ...
    try:
        ...
        if not extracted_text.strip():
            raise HTTPException(
                status_code=400, detail="Could not extract text from this PDF file."
            )
...
def get_personalized_feed(student_id: str):
    ...
    personalized_feed.sort(
        key=lambda x: (x["is_mandatory"] == False, -x["match_metrics"]["score"])
    )
